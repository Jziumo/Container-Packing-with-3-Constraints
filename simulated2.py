...
def simulatedAnnealing(task, num_iter = 100000, print_out=False, cooling_rate=0.98, initial_temperature=100000000):
    
    data = read_data.readSortedData(task=task, w=1, v=201, p=1, ascending=False, print_out=False)

    solution, initial_num_containers = greedy.greedy(task=task, data=data, print_out=False)
    # solution, num_containers = random_solution.getRandomSolution(task='a', print_out=False)
    ...
    # turns the solution list into a min-heap
    # each entry [rate of container, container]
    ...
    # these variables may not be used
    ...
    for iter_cnt in range(num_iter):
        ...
        if changed == True:
            ...
            if avg_rate > current_rate or (num_containers + len(containersBlocked)) < current_num_containers:
                ...
                best_num_containers = num_containers + len(containersBlocked)
                bestSolution = solution.copy()
    ...
    print(f'beat best? {beat_best}')

    print(f'best number of containers: {best_num_containers} | initial number of containers: {initial_num_containers}')

    for i in range(len(containersBlocked)):
        bestSolution.append(containersBlocked[i])

    check_res = check.check(solution=bestSolution)
    print(check_res)
...
simulatedAnnealing(task='a', num_iter=10000000000, print_out=False, initial_temperature=1000000000, cooling_rate=0.999)