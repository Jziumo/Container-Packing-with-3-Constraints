...
def testSizeLimit(task):
    ...
    for i in range(1, size):
        data = read_data.readSortedData(task=task, w=1, v=1, p=1, first_n_rows=i)

        res, _ = exact_search.exactSearch(task=task, data=data, print_out=False, greedy_hint=True)
...
def testSize(task, size):
    data = read_data.readSortedData(task=task, w=1, v=1, p=1, ascending=False, first_n_rows=size)
    ...
    res, _ = exact_search.exactSearch(task=task, data=data, print_out=True, greedy_hint=True)
